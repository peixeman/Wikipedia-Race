...
class WikiRaceClient:
    ...
    def discover_servers(self, timeout=5):
        """Auto-discover servers on LAN via UDP broadcast"""
        servers = []
        ...
        while time.time() - start_time < timeout:
            try:
                ...
                if message.get("type") == "server_discovery":
                    lobby = message.get("lobby_code")
                    if lobby not in discovered:
                        discovered.add(lobby)
                        servers.append({
                            "lobby_code": lobby,
                            "ip": message.get("ip"),
                            "port": message.get("port")
                        })
                        print(f"Discovered server: {lobby} at {message.get("ip")}")
    ...
    def connect_to_server(self, lobby):
        ...
        # Discover servers
        self.update_status("Searching for servers...")
        servers = self.discover_servers(timeout=5)

        # Find matching server
        server = None
        for s in servers:
            if s["lobby_code"] == lobby.upper():
                server = s
                break

        if not server:
            self.update_status(f"Error: Lobby \"{lobby}\" not found!")
            return False

        # Connect via TCP
        try:
            self.server_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            self.server_socket.connect((server["ip"], server["port"]))
    ...
    def play_game(self, start_article, end_article):
        ...
        # Create a simple root window to keep the event loop alive
        # while waiting for results from the server
        self.root = customtkinter.CTk()
        self.root.withdraw()  # Hide the window
        self.root.mainloop()  # This will exit when results window is closed
